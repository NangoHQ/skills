...
- In a full refresh, call `saveCheckpoint()` after every successful page, including the last, before calling `clearCheckpoint()`. Never guard the save with "more pages remain" or work around a skipped terminal save by conditionally skipping `clearCheckpoint()`; clearing a nonexistent checkpoint fails with `checkpoint_conflict`.
