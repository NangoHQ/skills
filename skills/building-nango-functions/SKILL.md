...
- `saveCheckpoint()` guarded so it only runs when more pages remain (e.g. `if (nextCursor) { await nango.saveCheckpoint(...) }`), skipping it on the last page — the very next call, `clearCheckpoint()`, then has no checkpoint to delete
- calling `clearCheckpoint()` unconditionally without confirming a checkpoint was actually saved this run or resumed from a prior one — `clearCheckpoint()` is optimistic-locked and rejects a delete against a nonexistent row with the same `checkpoint_conflict` error as a genuine concurrent write, most often surfacing on syncs whose entire dataset fits in a single page
...
- `clearCheckpoint()` deletes with an optimistic-lock check; if no checkpoint row exists (the run never called `saveCheckpoint()` — e.g. the whole dataset fit on the first page and progress is only saved when more pages remain), it fails with `checkpoint_conflict` exactly as if a concurrent write had raced it. Track whether this run saved a checkpoint (or resumed one via `getCheckpoint()`), and only call `clearCheckpoint()` when one actually exists to delete.
