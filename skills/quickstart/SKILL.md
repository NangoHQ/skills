...
- `POST /connect/sessions`: create the authorization link. Body: `allowed_integrations: ["<integration-id>"]` and `tags: { "end_user_id": "nango-onboarding-<timestamp>" }`. Return `data.connect_link` to the user. The link expires at `data.expires_at`. Optional `integrations_config_defaults: { "<integration-id>": { "connection_config": { "webhook_url": "<local-tunnel-url>" } } }` routes this connection's Nango webhooks to a local tunnel (e.g. ngrok) for local development instead of the environment webhook URLs. See `https://nango.dev/docs/implementation-guides/platform/webhooks-from-nango#override-webhook-urls-per-connection`.
...
- Remote function flow: compile with `POST /functions/compile`, start an async dryrun with `POST /functions/dryruns`, poll `GET /functions/dryruns/{id}` until `success` or `failed`, start async deployment with `POST /functions/deployments`, then poll `GET /functions/deployments/{id}` until `success` or `failed`. Compile catches code errors, dryrun tests against the connection, and successful deployment makes the function usable by the rest of Nango and the SDK.
