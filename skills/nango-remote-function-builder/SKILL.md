...
The function will be deployed using `sf-deploy` endpoint, check references/api.md. Once deployed we should run it with `sf-run`

## When API Docs Do Not Render

If web fetching returns incomplete docs (JS-rendered):
- Ask the user for a sample response
- Do a best effort apporach to `sf-deploy` and `sf-run` to test it
